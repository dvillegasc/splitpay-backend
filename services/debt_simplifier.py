...
def calculate_member_balances(db: Session, household_id: UUID) -> Dict[UUID, Decimal]:
    ...
    Si la moneda del gasto difiere de la moneda base del hogar, el monto se convierte
    a la moneda base utilizando `convert_amount()`.
    ...
    for expense in approved_expenses:
        ...
        if expense.moneda != household.moneda_base:
            monto_pagador = convert_amount(
                expense.monto_total, expense.moneda, household.moneda_base
            )
        else:
            monto_pagador = expense.monto_total

        if pagador_id in balances:
            balances[pagador_id] += monto_pagador
        else:
            balances[pagador_id] = monto_pagador
        ...
        for split in splits:
            if expense.moneda != household.moneda_base:
                monto_split = convert_amount(
                    split.monto_asignado, expense.moneda, household.moneda_base
                )
            else:
