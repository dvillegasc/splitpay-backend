...
- [ ] Backend: Crear `services/currency_converter.py` que integre una API pública de tasas de cambio en tiempo real (ej. exchangerate.host) con una función `convert_amount(amount, from_currency, to_currency) -> Decimal`, cacheando las tasas en memoria por un tiempo configurable para no exceder los límites de la API.
- [ ] Backend: En `routers/expenses.py`, reemplazar el rechazo (400) de gastos con `moneda` distinta a `household.moneda_base` por una conversión real usando `services/currency_converter.py`, agregando un campo `monto_total_moneda_base` a `Expense` (y su migración) para preservar tanto el monto original como el convertido.
- [ ] Backend: Crear endpoint `PUT /api/expenses/{expense_id}/reject` en `routers/expenses.py` que marque `estado_aprobacion` como `RECHAZADO`, requiriendo que el usuario autenticado tenga una cuota asignada en el gasto o sea el pagador.
- [ ] Backend: Crear endpoint `DELETE /api/households/{household_id}/members/{user_id}` en `routers/households.py` que elimine la membresía de un usuario en el hogar, validando que el solicitante sea miembro y reasignando/limpiando el flag de tesorero si el miembro removido lo tenía asignado.
- [ ] Backend: Generar la migración inicial real de Alembic ejecutando `alembic revision --autogenerate -m "esquema inicial"` y comitear el archivo resultante en `alembic/versions/`, ya que actualmente no existe ningún archivo de migración versionado en el repositorio.
- [ ] Backend: Crear `tests/test_math_engine.py` con pruebas unitarias en `pytest` para `calculate_proportional_split`, cubriendo: todos los ingresos en cero, un solo miembro con ingreso, ingresos negativos, y verificación de que la suma de `monto_asignado` siempre sea exactamente igual a `total_amount`.
